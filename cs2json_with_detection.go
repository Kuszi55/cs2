...
type SuspiciousMoment struct {
	...
	RelatedPlayerName  string  `json:"relatedPlayerName,omitempty"`
}
...
type PlayerHistory struct {
	...
	Name      string
	Team      common.Team
	Snapshots map[int]*PlayerSnapshot
	Kills     []KillEvent
	Damages   []DamageEvent
}
...
func main() {
	...
	parser.RegisterEventHandler(func(e events.Kill) {
		summary.TotalKills++
		lastKillTick = e.GameState().IngameTickCount()
		...
		if e.Killer != nil {
			...
			killerHist.Kills = append(killerHist.Kills, KillEvent{
				...
				Victim:   e.Victim.Name if e.Victim != nil else "Unknown",
				Weapon:   e.Weapon.String() if e.Weapon != nil else "Unknown",
		...
		if len(playerHist.Damages) > 0 {
			...
			for i := 0; i < len(damages)-1; i++ {
				...
				if burstDamage > 150 && burstEnd-burstStart > 10 {
					...
					moments = append(moments, SuspiciousMoment{
...
func getOrCreateHistory(steamID uint64, history map[uint64]*PlayerHistory) *PlayerHistory {
	...
	newHist := &PlayerHistory{
		SteamID:   steamID,
		Snapshots: make(map[int]*PlayerSnapshot),
